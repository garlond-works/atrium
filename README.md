...
- 1日に聞ける回数は、`wrangler.jsonc` の `ASK_PER_DAY` で自由に決められます（1つの応接室あたり・空か 0 なら上限なし）。無料枠をどう配るかに合わせて決めてください
...
- 書き込み（伝言・資料を置く など）は Atrium 自身の画面からだけ受け付けます（よそのサイトからの送信＝CSRF を拒否）
- 置ける資料は1ファイル50MBまで、伝言は4,000字までです
